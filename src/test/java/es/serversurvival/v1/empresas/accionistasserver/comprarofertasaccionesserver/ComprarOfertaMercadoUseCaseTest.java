package es.serversurvival.v1.empresas.accionistasserver.comprarofertasaccionesserver;
...
import es.serversurvival.v1.empresas.ofertasaccionesserver.comprarofertasaccionesserver.ComprarOfertaMercadoUseCase;
import es.serversurvival.v1.empresas.ofertasaccionesserver.comprarofertasaccionesserver.EmpresaServerAccionComprada;
...
public final class ComprarOfertaMercadoUseCaseTest {
    ...
    public void shouldComprarPartEmpresa(){
        ...
        OfertaAccionServer oferta = OfertasAccionesServerTestMother.createOfertAccionServerEmpresa("empresa", "empresa", 20, 2);
        Empresa empresa = createEmpresa("empresa", "jaime");
        Jugador comprador = JugadoresTestMother.createJugador("comprador", 50);
        ...
        verify(this.accionistasServerService, times(1)).save(
                argThat(of("comprador")), argThat(of(TipoAccionista.JUGADOR)), argThat(of("empresa")), anyInt(), anyDouble()
        );
        ...
        verify(this.eventBus, times(1)).publish(argThat(of(
                EmpresaServerAccionComprada.of("comprador", "empresa", TipoAccionista.EMPRESA, "empresa", 20, 1)
        )));
    }
    ...
    public void shouldComprarFullJugador(){
        UUID idToTest = UUID.randomUUID();
        OfertaAccionServer oferta = OfertasAccionesServerTestMother.createOfertAccionServerJugador("vendedor", "empresa", 20, 2);
        Jugador comprador = JugadoresTestMother.createJugador("comprador", 50);
        Jugador vendedor = JugadoresTestMother.createJugador("vendedor", 50);
        ...
        verify(this.eventBus, times(1)).publish(argThat(of(
                EmpresaServerAccionComprada.of("comprador", "vendedor", TipoAccionista.JUGADOR, "empresa", 20 * 2, 2)
        )));
    }
    ...
    public void enoughPixelcoins(){
        UUID idToTest = UUID.randomUUID();
        OfertaAccionServer oferta = OfertasAccionesServerTestMother.createOfertAccionServerJugador("vendedor", "empresa", 20, 10);
        Jugador comprador = JugadoresTestMother.createJugador("comprador", 100);
        when(this.jugadoresService.getByNombre("comprador")).thenReturn(comprador);
        when(this.ofertasAccionesServerService.getById(idToTest)).thenReturn(oferta);
    ...
    public void notHisSelf(){
        UUID idToTest = UUID.randomUUID();
        when(this.jugadoresService.getByNombre("comprador")).thenReturn(JugadoresTestMother.createJugador("comprador"));
        when(this.ofertasAccionesServerService.getById(idToTest)).thenReturn(OfertasAccionesServerTestMother.createOfertAccionServerJugador("comprador", "!2"));
    ...
    public void cantidadCorrectQuantity(){
        UUID idToTest = UUID.randomUUID();
        OfertaAccionServer oferta = OfertasAccionesServerTestMother.createOfertAccionServerJugador("comprador", ":)");
        when(this.jugadoresService.getByNombre("comprador")).thenReturn(JugadoresTestMother.createJugador("comprador"));
        when(this.ofertasAccionesServerService.getById(idToTest)).thenReturn(oferta);
