package es.serversurvival.v1.empresas.accionistasserver.venderofertaaccionaserver;
...
import es.serversurvival.v1.empresas.ofertasaccionesserver.venderofertaaccionaserver.NuevaOfertaAccionServer;
...
public final class VenderOfertaAccionServerUseCaseTest {
    ...
    public void shouldVender(){
        ...
        verify(this.ofertasAccionesServerService, times(1)).save(
                argThat(of("jaime")), argThat(of("empresa")), anyDouble(), anyInt(), argThat(of(TipoAccionista.JUGADOR)), anyDouble(),
                argThat(of(accionistaServer.getAccionistaServerId()))
        );

        verify(this.eventBus, times(1)).publish(argThat(of(
                NuevaOfertaAccionServer.of("jaime", "empresa", TipoAccionista.JUGADOR, 1, 1)
        )));
    }
