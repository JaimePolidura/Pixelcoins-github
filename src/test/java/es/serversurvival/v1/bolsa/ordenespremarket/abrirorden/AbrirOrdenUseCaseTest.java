...
public final class AbrirOrdenUseCaseTest {
    ...
    public void shouldCreateOrder(){
        ...
        this.useCase.abrirOrden(AbrirOrdenPremarketCommand.of("jaime", "AMZN", 1, TipoAccion.LARGO_COMPRA, posicionAbiertaId));
        ArgumentCaptor<Integer> argumentCaptor = ArgumentCaptor.forClass(Integer.class);

        verify(this.ordenesPremarketService, times(1)).save(
                argThat(of("jaime")), argThat(of("AMZN")), argumentCaptor.capture(), argThat(of(TipoAccion.LARGO_COMPRA)), argThat(of(posicionAbiertaId))
        );
        assertThat(argumentCaptor.getValue()).isEqualTo(1);

        verify(this.eventBus, times(1)).publish(argThat(of(
                OrdenAbiertaEvento.of("jaime", "AMZN", 1, TipoAccion.LARGO_COMPRA, posicionAbiertaId)
        )));
    }
    ...
    public void ordenAlreadyRegisteredForPosicionAbierta(){
        ...
        assertThatCode(() -> this.useCase.abrirOrden(AbrirOrdenPremarketCommand.of("jaime", "AMZN",
                1, TipoAccion.LARGO_COMPRA, posicionAbiertaId)))
    ...
    public void ownerOfPosicionAbierta(){
        UUID posicionAbiertaId = UUID.randomUUID();
        when(this.posicionesAbiertasSerivce.getById(posicionAbiertaId)).thenReturn(PosicionesAbiertasTestMother.createPosicionAbierta("otro", "AMZN"));
        assertThatCode(() -> this.useCase.abrirOrden(AbrirOrdenPremarketCommand.of("jaime", "AMZN",
                1, TipoAccion.LARGO_VENTA, posicionAbiertaId)))
