package es.serversurvival.empresas.accionistasserver.cancelarofertaccionserver;
...
public final class CancelarOfertaAccionServerUseCaseTest {
    ...
    public void shouldCancelarOferta(){
        ...
        when(this.ofertasAccionesServerService.getById(idOferta)).thenReturn(createOfertAccionServerJugador("jaime", "empresa"));
    ...
    public void ensureOwner(){
        UUID idOferta = UUID.randomUUID();
        when(this.ofertasAccionesServerService.getById(idOferta)).thenReturn(createOfertAccionServerJugador("otro", "empresa"));
