...
import static es.serversurvival.bolsa._shared.ordenespremarket.mysql.AccionOrden.*;
...
public class ComprarCortoComandoExecutor extends PixelcoinCommand implements CommandRunnerArgs<ComprarCortoComando> {
    ...
    public void execute(ComprarCortoComando comando, CommandSender sender) {
        ...
        if(Funciones.mercadoEstaAbierto()){
            ...
            abrirOrdenUseCase.abrirOrden(sender.getName(), posicionAComprar.getNombre_activo(), cantidad, CORTO_COMPRA, posicionAComprar.getId());
        }
    }
