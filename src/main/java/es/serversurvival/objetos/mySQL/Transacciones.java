...
import es.serversurvival.objetos.mySQL.tablasObjetos.Transaccion;
...
public class Transacciones extends MySQL {
    ...
    public void realizarVenta(String comprador, int id, Player p) {
        ...
        p.sendMessage(ChatColor.GOLD + "Has comprado: " + objeto + " , por " + ChatColor.GREEN + formatea.format(precio) + " PC" + ChatColor.GOLD + " .Te quedan: " +
                ChatColor.GREEN + formatea.format(dineroComprador - precio) + " PC");
            ...
            switch (tipo) {
                case "DIAMOND":
                    if (actualPixelcoins >= DIAMANTE) {
                        ...
                        p.sendMessage(ChatColor.GOLD + "Se ha a?adido un diamante. " + ChatColor.RED + "-" + DIAMANTE + " PC" + ChatColor.GOLD +
                                "Quedan " + ChatColor.GREEN + formatea.format(actualPixelcoins - DIAMANTE) + " PC");
                        p.playSound(p.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 10, 1);
                        break;
                    } else {
                ...
                case "QUARTZ_BLOCK":
                    if (actualPixelcoins >= CUARZO) {
                        ...
                        p.sendMessage(ChatColor.GOLD + "Se ha a?adido un bloque de cuarzo. " + ChatColor.RED + "-" + CUARZO + " PC" +
                                ChatColor.GOLD + "Quedan " + ChatColor.GREEN + formatea.format(actualPixelcoins - CUARZO) + " PC");
                        p.playSound(p.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 10, 1);
                        break;
                    } else {
                ...
                case "LAPIS_LAZULI":
                    if (actualPixelcoins >= LAPISLAZULI) {
                        ...
                        p.sendMessage(ChatColor.GOLD + "Se ha a?adido lapislazuli. " + ChatColor.RED + "-" + LAPISLAZULI + " PC" +
                                ChatColor.GOLD + "Quedan " + ChatColor.GREEN + formatea.format(actualPixelcoins - LAPISLAZULI) + " PC");
                        p.playSound(p.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 10, 1);
                        break;
                    } else {
    ...
    public void depositarPixelcoins(Player p, double pixelcoins, String nombreEmpresa) {
        ...
        sp.updateScoreboard(p);
        p.sendMessage(ChatColor.GOLD + "Has metido " + ChatColor.GREEN + formatea.format(pixelcoins) + " PC" + ChatColor.GOLD
    ...
    public void sacarPixelcoins(Player p, double pixelcoins, String nombreEmpresa) {
        ...
        p.sendMessage(ChatColor.GOLD + "Has sacado " + ChatColor.GREEN + formatea.format(pixelcoins) + " PC" + ChatColor.GOLD
    ...
    public void comprarUnidadBolsa(String tipo, String ticker, String alias, double precioUnidad, int cantidad, Player player) {
        ...
        if (precioTotal > pixelCoinsJugador) {
            player.sendMessage(ChatColor.DARK_RED + "No tienes las suficientes pixelcoins para pagar " + cantidad + " " +
                    alias + " a " + formatea.format(precioUnidad) + " $ -> " + formatea.format(precioTotal) + " PC");
            return;
        }
        j.setPixelcoin(player.getName(), pixelCoinsJugador - precioTotal);
        posicionesAbiertas.nuevaPosicion(player.getName(), tipo, ticker, cantidad, precioUnidad);
        this.nuevaTransaccion(player.getName(), ticker, precioTotal, tipo +  " " + precioUnidad, TIPO.BOLSA_COMPRA);
        ...
        if (player != null) {
            ...
            Bukkit.broadcastMessage(ChatColor.GOLD + player.getName() + " ha comprado " + cantidad + " " + alias +  " de " + ticker + " a " + ChatColor.GREEN + precioUnidad + "PC");
    ...
    public void venderPosicion(double precioPorAccion, int cantidad, int idPosiconAbierta, Player player) {
        ...
        String ticker = posicionesAbiertas.getNombre(idPosiconAbierta);
        int nAccionesTotlaesEnCartera = posicionesAbiertas.getCantidad(idPosiconAbierta);
        ...
        String tipoPosicion = posicionesAbiertas.getTipo(idPosiconAbierta);
        ...
        if (cantidad == nAccionesTotlaesEnCartera) {
            ...
            posicionesAbiertas.setCantidad(idPosiconAbierta, nAccionesTotlaesEnCartera - cantidad);
        }
        posicionesCerradas.nuevaPosicion(player.getName(), tipoPosicion, ticker, cantidad, precioApertura, fechaApertura, precioPorAccion);

        this.nuevaTransaccion(ticker, player.getName(), cantidad * precioPorAccion, "", TIPO.BOLSA_VENTA);

        if(!posicionesAbiertas.existeTicker(ticker)){
            LlamadasApi llamadasApi = new LlamadasApi();
            llamadasApi.borrarLlamada(ticker);
        }
    ...
    public List<Transaccion> getTopVentasJugador(String player, int limite){
        List<Transaccion> transacciones = new ArrayList<>();

        try{
            String cosulta = "SELECT * FROM transacciones WHERE vendedor = ? AND tipo = ? ORDER BY cantidad DESC LIMIT " + limite;
            PreparedStatement preparedStatement = conexion.prepareStatement(cosulta);
            preparedStatement.setString(1, player);
            preparedStatement.setString(2, "TIENDA");
